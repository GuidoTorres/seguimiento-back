...
SELECT OA.ANO_EJE, D.TIPO_BIEN, OA.NRO_ORDEN, OA.FECHA_ORDEN, OA.MES_CALEND, OA.EXP_SIAF,OA.CONCEPTO, OA.ESTADO, OIP.FUENTE_FINANC, OIP.SEC_FUNC, OIP.CLASIFICADOR, CCBS.NOMBRE_CLASIF, M.ESTRATEGIA_NACIONAL, M.act_proy, M.componente, M.funcion, M.programa, M.sub_programa, M.finalidad, D.SECUENCIA, C.NRO_RUC, C.NOMBRE_PROV, 
 DBC.GRUPO_BIEN, DBC.CLASE_BIEN, DBC.FAMILIA_BIEN, DBC.ITEM_BIEN, CBS.NOMBRE_ITEM, DBC.UNIDAD_MEDIDA,  D.CENTRO_COSTO, CC.NOMBRE_DEPEND,  D.TIPO_TAREA, D.NIVEL_TAREA,D.CODIGO_TAREA, T.nombre, D.CANT_DEPEND, D.VALOR_DEPEND, OI.PREC_UNIT_MONEDA, OI.CANT_ITEM, OI.PREC_TOT_SOLES, OA.TOTAL_FACT_SOLES, OA.NRO_CERTIFICA, OI.CUSER_ID
FROM SIG_ORDEN_ADQUISICION AS OA INNER JOIN SIG_DEPEN_META_CUADRO AS D ON (OA.SEC_CUADRO=D.SEC_CUADRO  AND OA.ANO_EJE=D.ANO_EJE AND OA.SEC_EJEC=D.SEC_EJEC AND OA.TIPO_BIEN = D.TIPO_BIEN)
...
WHERE OA.ANO_EJE='2024' AND OA.TIPO_BIEN='S' AND C.TIPO_PERSONA='05'
GROUP BY OA.ANO_EJE, D.TIPO_BIEN, OA.NRO_ORDEN, OA.FECHA_ORDEN, OA.MES_CALEND, OA.EXP_SIAF,OA.CONCEPTO, OA.ESTADO, OIP.FUENTE_FINANC, OIP.SEC_FUNC, OIP.CLASIFICADOR, CCBS.NOMBRE_CLASIF, M.ESTRATEGIA_NACIONAL, M.act_proy, M.componente, M.funcion, M.programa, M.sub_programa, M.finalidad, D.SECUENCIA, C.NRO_RUC, C.NOMBRE_PROV, 
 DBC.GRUPO_BIEN, DBC.CLASE_BIEN, DBC.FAMILIA_BIEN, DBC.ITEM_BIEN, CBS.NOMBRE_ITEM, DBC.UNIDAD_MEDIDA,  D.CENTRO_COSTO, CC.NOMBRE_DEPEND,  D.TIPO_TAREA, D.NIVEL_TAREA,D.CODIGO_TAREA, T.nombre, D.CANT_DEPEND, D.VALOR_DEPEND, OI.PREC_UNIT_MONEDA, OI.CANT_ITEM, OI.PREC_TOT_SOLES, OA.TOTAL_FACT_SOLES, OA.NRO_CERTIFICA,  OI.CUSER_ID
...
const registrarServiciosEnBd = async (req, res, next) => {
  try {
    const [resultados, metadata] = await sequelize.query(`
SELECT OA.ANO_EJE, D.TIPO_BIEN, OA.NRO_ORDEN, OA.FECHA_ORDEN, OA.MES_CALEND, OA.EXP_SIAF,OA.CONCEPTO, OA.ESTADO, OIP.FUENTE_FINANC, OIP.SEC_FUNC, OIP.CLASIFICADOR, CCBS.NOMBRE_CLASIF, M.ESTRATEGIA_NACIONAL, M.act_proy, M.componente, M.funcion, M.programa, M.sub_programa, M.finalidad, D.SECUENCIA, C.NRO_RUC, C.NOMBRE_PROV, 
 DBC.GRUPO_BIEN, DBC.CLASE_BIEN, DBC.FAMILIA_BIEN, DBC.ITEM_BIEN, CBS.NOMBRE_ITEM, DBC.UNIDAD_MEDIDA,  D.CENTRO_COSTO, CC.NOMBRE_DEPEND,  D.TIPO_TAREA, D.NIVEL_TAREA,D.CODIGO_TAREA, T.nombre, D.CANT_DEPEND, D.VALOR_DEPEND, OI.PREC_UNIT_MONEDA, OI.CANT_ITEM, OI.PREC_TOT_SOLES, OA.TOTAL_FACT_SOLES, OA.NRO_CERTIFICA, OI.CUSER_ID
        ...
        include: [
          {
            ...
            required: true, // Esto asegura que solo se incluyan registros que tienen un estado
            where: {
              nombre: "Recepcionado", // Filtrar los que tienen el estado "Recepcionado"
              completado: false,
            },
          },
        ],
...
const actualizarEstadoRecepcionado = async (req, res) => {
  try {
    const token = req.cookies.tokenSession;
    if (!token) {
      return res.status(403).json({ msg: "No autorizado, token faltante" });
    ...
    return res
      .status(200)
      .json({ msg: `Se recepciono correctamente la orden Nro ${nro_orden}.` });
  } catch (error) {
...
const actualizarEstadoConformidad = async (req, res) => {
  try {
    const token = req.cookies.tokenSession;
    if (!token) {
      return res.status(403).json({ msg: "No autorizado, token faltante" });
    ...
    return res
      .status(200)
      .json({ msg: `Se registro la conformidad de la orden Nro ${nro_orden}.` });
  } catch (error) {
